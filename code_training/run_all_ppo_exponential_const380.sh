...
CONFIG_DIR="${CONFIG_DIR:-conf/config_PPO_exponential_const380}"
...
mapfile -t CONFIG_FILES < <(find "$CONFIG_DIR" -maxdepth 1 -name 'config_PPO_expo*_const380.yaml' | sort)

if [ "${#CONFIG_FILES[@]}" -eq 0 ]; then
    echo "Error: no config_PPO_expo*_const380.yaml files found in $CONFIG_DIR"
    exit 1
fi

echo "Running PPO exponential const380 configs serially"
...
PY
    ...
    if [ "$DRY_RUN" = "1" ]; then
        echo "Would run: python main.py --config-path conf/config_PPO_exponential_const380 --config-name ${config_name}"
        continue
    fi

    echo "Starting training..."
    python main.py --config-path conf/config_PPO_exponential_const380 --config-name "${config_name}"
...
echo
echo "================================================"
echo "All PPO exponential const380 configs processed."
