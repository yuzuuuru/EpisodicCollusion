"""実験結果から Collusion Index 関連の可視化を作るスクリプト。

このスクリプトは `--save_dir` で指定した実験ディレクトリごとに、以下を行います。
1. `args.pkl` / `log_data.pkl` / `update_dict.pkl` / `seeds.pkl` を読み込む
2. 学習中の総利得・平均アクション・Collusion Index（算術平均/一般化平均）を集計する
3. 図を `<save_dir>/paper_plots/` に保存する
   - `all_plots.png`（4パネルの概要図）
   - `fig2_<run_name>_collindex.png`（Collusion Index の縦長図）

主な用途:
- DQN/PPO などの学習結果を seed 平均と分散付きで比較する
- 学習後半 10% の一般化平均 Collusion Index を確認する
- actionの履歴は保存せず、Collution Indexのみを保存するスクリプト

実行例:
`python code_training/make_collindex.py --save_dir exp/DQN exp/PPO`
"""
...
def plot_vert(env_metrics, x_axis, gen_mean_p):
    ...
    # )
    ax.plot(filtered_x_axis, coll_idx_gen_mean, linewidth=2, color=collusion_color)
    ax.fill_between(
        filtered_x_axis,
        coll_idx_gen_mean - np.sqrt(coll_idx_gen_var),
        coll_idx_gen_mean + np.sqrt(coll_idx_gen_var),
    ...
    # ax.set_title(f"Collusion Index (Generalized Mean, p={generalized_mean_p})")
    ...
    ax.set_xlabel("Episodes", fontsize=15, labelpad=10)
    ax.set_ylabel("Collusion Index", fontsize=15, labelpad=10)
    ...
    fig.tight_layout()
    plt.savefig(
        os.path.join(plot_dir, f"fig2_{run_name}_collindex.png"),
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
    )
    print(f"saved fig2_{run_name}_collindex.png")
    plt.close()
    return fig, ax
    ...
    for sd in cli_args.save_dir:
        ...
        try:
            ...
            try:
                ...
                display_single_plot(os.path.join(save_dir, "paper_plots", f"fig2_{run_name}_collindex.png"))
            except Exception as e:
