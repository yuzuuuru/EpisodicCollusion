...
CONFIG_DIR="${CONFIG_DIR:-conf/config_DQN_exponential}"
...
mapfile -t CONFIG_FILES < <(find "$CONFIG_DIR" -maxdepth 1 -name 'config_DQN_expo*.yaml' | sort)

if [ "${#CONFIG_FILES[@]}" -eq 0 ]; then
    echo "Error: no config_DQN_expo*.yaml files found in $CONFIG_DIR"
    exit 1
fi

echo "Running DQN exponential configs serially"
...
PY
    ...
    if [ "$DRY_RUN" = "1" ]; then
        echo "Would run: python main.py --config-path conf/config_DQN_exponential --config-name ${config_name}"
        continue
    fi

    echo "Starting training..."
    python main.py --config-path conf/config_DQN_exponential --config-name "${config_name}"
...
echo
echo "================================================"
echo "All DQN exponential configs processed."
