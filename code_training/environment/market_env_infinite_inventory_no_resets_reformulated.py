"""
MarketEnvInfiniteInventoryInfiniteEpisodeReformulated: Infinite episode environment with observation reformulation.

This environment is designed for the infinite episode setting without inventory constraints.
Key differences from MarketEnvReformulated:
1. Inventory is never decremented (infinite inventory)
2. Done signal is effectively always False (infinite episode)

NOTE: Training is controlled by `num_iters` in the config file, NOT by the environment's done signal.
The done condition here (t >= 10000000000) ensures the environment never signals episode termination,
as the actual training loop length is determined externally by the runner using num_iters.
"""
...
class EnvParams:
    ...
    # 0 = none/original, 1 = min_max_mean, 2 = top_k, 3 = set_top_k
...
# region ENVIRONMENT
class MarketEnvInfiniteInventoryInfiniteEpisodeReformulated(environment.Environment):
    """Infinite episode environment with observation reformulation support.
    
    This environment is meant to be used in the infinite episode setting:
    1. Inventory is never decremented (infinite inventory)
    2. Done signal is effectively always False (infinite episode)
    
    NOTE: Training duration is controlled by `num_iters` in config, not by done signal.
    The environment's done condition (t >= 10000000000) ensures it never terminates,
    while the runner controls the actual training length via num_iters.
    """
    ...
    def __init__(self, num_agents: int, num_actions: int, time_horizon: int, 
        ...
        def _step(
            ...
            # NOTE: done is effectively always False for infinite episode setting.
            # Training duration is controlled by num_iters in config, not by this done signal.
            # We use jnp.int32 max value to ensure JAX compatibility while keeping done always False.
            done = jnp.squeeze(t) >= jnp.iinfo(jnp.int32).max
            ...
            # KEY DIFFERENCE: Inventory is NOT decremented (infinite inventory)
            # This is the main difference from MarketEnvReformulated
            inventories = state.inventories  # Keep inventory unchanged
    ...
    @property
    def name(self) -> str:
        return "MarketEnv-InfiniteInventoryInfiniteEpisode-Reformulated"
