...
In recent years, firms have increasingly adopted algorithms for pricing products and services\cite{Chen2016}.
Against the backdrop of expanding e-commerce and increasingly sophisticated data-processing infrastructure, dynamic pricing that flexibly adjusts prices in response to demand and inventory conditions has become more important.
In such environments, fixed pricing rules designed manually are often insufficient to respond quickly to environmental changes, and data-driven approaches based on machine learning have therefore attracted growing attention.

At the same time, the autonomy of learning algorithms has raised concerns that they may sustain higher prices even in the absence of explicit communication among competing firms, thereby giving rise to tacit collusion through algorithms\cite{mehra2016,ezrachi2017}.
Such outcomes may reduce consumer welfare and market efficiency, and they also create enforcement challenges because they are difficult to reconcile with the traditional competition-policy focus on explicit agreements or communication\cite{harrington2019}.
For these reasons, understanding the conditions under which algorithms converge to collusive outcomes in competitive environments is of both academic and social importance.

For example, Calvano et al.\cite{calvano2020a} analyzed reinforcement-learning agents in repeated Bertrand competition and showed that the agents can endogenously develop coordinated behavior supported not only by elevated prices but also by reward-punishment structures in response to deviations.
% この知見は，プラットフォーム設計や規制設計への含意を持ち，たとえば
This line of work has since been extended to questions of institutional design from the perspective of consumer and platform welfare\cite{johnson2020}.

In particular, Friedrich et al.\cite{friedrich2025learning} showed that repeated learning by agents can produce collusive pricing in finite-horizon, inventory-constrained episodic markets.
In that study, as in many related settings, demand scale is assumed to remain constant throughout the episode.
In real reservation markets, however, demand scale evolves over time.
For example, airline markets exhibit seasonal demand fluctuations, while hotel markets often experience sharp increases or decreases in demand around event periods.
Such dynamic changes in demand scale may substantially affect firms' optimal pricing strategies and the mechanisms that sustain tacit collusion.

This paper makes two main contributions.
First, we extend the episodic inventory-constrained market model in prior work by replacing the constant demand-scale factor $\lambda$ with a time-dependent demand-scale factor $\lambda_t$.
This yields a framework that captures temporal demand variation observed in reservation markets.
Importantly, this extension preserves a structure in which Nash benchmark prices can still be computed within practical time limits even when demand scale varies over time, which enables numerical comparisons across different market scenarios.
Second, using this framework, we incorporate temporal demand structure observed in reservation markets into the analysis of reinforcement-learning-based algorithmic collusion and study its effects experimentally.
Our results show that temporal variation in demand scale affects the formation of collusion, and that the converged collusion index tends to be lower when demand scale changes more sharply.
Moreover, the response to demand variation differs across learning algorithms: in particular, PPO exhibits substantial changes in both the level of collusion and the learned pricing strategies depending on how demand varies over time.
Within the scope of our model, these findings suggest that the temporal structure of demand in reservation markets can play an important role in the pricing behavior learned by agents, and that temporal smoothing of demand may affect the collusion level that emerges through learning.
...
\end{equation}
Figure~\ref{fig:episodic_collusion_model} illustrates this sequence of events: price posting at each time step, demand realization, inventory-constrained sales, inventory updating, and profit realization.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{episodiccollusionmodel2.png}
\caption{Overview of the interaction in an episodic inventory-constrained market: firms simultaneously choose prices at each time step; after demand is realized, sales are determined subject to inventory constraints, and inventories and profits are updated\cite{friedrich2025learning}.}
...
We explain this argument below.
First, consider the continuous-demand approximation obtained by removing the floor function,
$
\tilde d_{i,t}:=\lambda_t s_{i,t}
\label{eq:scaled_demand_relaxed}
$
Then, given the rivals' price paths, the optimization problem of firm $i$ can be written as follows: % 注:この最適化問題とは利益の最大化ではなく、ナッシュ均衡価格を求めるための最適化問題である。
\begin{equation}
...
\end{equation}
Let $\nu_i\ge 0$ be the Lagrange multiplier associated with the inventory constraint.
...
\end{equation}
Because $\lambda_t>0$, for any fixed $\nu_i$ the problem of choosing the period-$t$ price $p_{i,t}$ is equivalent to maximizing
\begin{equation}
\bigl((p_{i,t}-c_i)-\nu_i\bigr)s_{i,t}
\label{eq:timevarying_lambda_inner_problem}
\end{equation}
so $\lambda_t$ itself does not change the ordering of prices in terms of optimality.
Therefore, under the continuous-demand approximation, $\lambda_t$ does not alter the ranking of candidate prices in each period, and when the comparison is made under
\begin{equation}
\sum_{t=1}^{T}\lambda_t = T\bar{\lambda}
\label{eq:timevarying_lambda_sum}
\end{equation}
as in our setting, the Nash benchmark price from the fixed-demand case with $\lambda=\bar{\lambda}$ can be used as a common benchmark.
...
\subsection{Market-Size Scenario Design}
...
\begin{figure}[t]
    ...
    \caption{Examples of exponential demand schedules $\lambda_t=a\exp(b(t-1))$, with $b$ adjusted so that $\sum_{t=1}^{20}\lambda_t=20000$.}
...
Figure~\ref{fig:ppo_forced_deviation_compare} further shows the price response when one agent is forced to deviate.
...
\begin{figure}[tb]
  ...
  \end{minipage}
  \caption{Comparison of price responses when one agent is forced to deviate. Top: deviation at $t=1$. Bottom: deviation at $t=9$. Left: fixed $\lambda$. Right: exponentially increasing $\lambda$.}
...
\begin{table}[tb]
\centering
\caption{Collusion index under exponential demand schedules (Eq.~\eqref{eq:collusion_index}, generalized mean with $\gamma=0.5$, DQN/PPO, 100 seeds).}
