...
%
\begin{abstract}
Dynamic pricing, in which agents actively learn pricing strategies for products and services, is increasingly being deployed. Simulation studies have shown that the autonomy of such agents can sustain price increases across competing firms without explicit communication, a phenomenon known as algorithmic collusion. Prior work on inventory-constrained pricing markets has typically assumed that demand scale remains constant over time. In this paper, we study a more realistic setting by introducing a market environment in which demand scale varies over time and examining how this affects algorithmic collusion. Our framework preserves computable competitive and collusive benchmarks, enabling numerical comparison across demand scenarios. In experiments with Proximal Policy Optimization (PPO), we focus on a negative correlation between the rate of change in demand scale and the resulting collusion index. These findings suggest that the temporal structure of demand plays an important role in the emergence of collusive behavior among learning agents.
...
More recent work has also analyzed collusive pricing and differences in its degree in deep-reinforcement-learning-based pricing games using PPO\cite{schulman2017proximal}, DQN, and related methods\cite{Schlechtinger2024,deng2024algorithmic}.
...
Most of these studies focus on only the prices determined by algorithms, whereas many real markets are subject to inventory constraints.
Friedrich et al.\cite{friedrich2025learning} showed that when learning algorithms focus on both prices and inventories across repeated episodes, they can experimentally acquire collusive behavior in finite-horizon inventory-constrained markets.
...
In practice, these benchmark prices are obtained numerically by solving the corresponding episode-level optimization problems.
...
%We explain this argument briefly. 
Removing the floor function gives the continuous-demand approximation $\tilde d_{i,t}:=\lambda_t s_{i,t}$. Given the rivals' price paths, firm $i$ solves
\begin{align}
...
Accordingly, we retain the competitive and collusive prices from the fixed-demand case as common benchmarks across all scenarios, and treat temporal variation in $\lambda_t$ primarily as an extension that changes how total demand is allocated across time within an episode.

\subsection{Demand-Scale Scenario Design}
To represent reservation-market demand patterns consistent with the ABCDEF law\cite{ShintaniUmeno2023ABCDEF}, we define the demand-scale factor $\lambda_t$ by
\begin{equation}
\lambda_t = a\exp(b(t-1))
\label{eq:lambda_exp}
\end{equation}
where $a>0$ denotes the initial demand scale and $b\in\mathbb{R}$ is the growth-rate parameter. We compare scenarios under a common total-demand level, choosing $(a,b)$ so that
\begin{equation}
\sum_{t=1}^{20} a\exp(b(t-1)) = 20000
\label{eq:lambda_sum_constraint}
\end{equation}
Thus, once $a$ is chosen, the value of $b$ is uniquely determined. Figure~\ref{fig:visualize_demand_curve} shows the resulting schedules for the seven values of $a$ used in our experiments. Smaller values of $a$ produce increasing-demand scenarios ($b>0$), larger values produce decreasing-demand scenarios ($b<0$), and $a=1000$ gives the constant-demand case. In our setting, $\lambda_t$ is not directly observed by the agents. Instead, agents learn while implicitly inferring the demand level from the time step $t$ and from observed outcomes such as sales and inventory. This assumption reflects reservation markets in which demand levels vary exogenously and are not fully observable.
...
One possible explanation is that DQN updates Q-values from step-level transitions, so the pattern of demand-scale variation over the entire episode is reflected less directly in per-step decisions.
...
Because PPO updates policies from episode-level trajectories, the value of coordination in the first half of the episode is evaluated as relatively small when the reward scale in the early phase (where $\lambda$ is small) is much lower than in the later phase.
...
Figure~\ref{fig:ppo_forced_deviation_compare} compares representative forced-deviation responses with a deviation imposed at $t=1$ in the fixed-$\lambda$ case (left) and the exponentially increasing-$\lambda$ case (right).
...
\begin{figure}[tb]
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{fig3a_PPO-1000-fixed_forced_dev_cropped.png}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{fig3a_PPO-1000-exponential-400_1984.524399_forced_dev_cropped.png}
  \end{minipage}
  \caption{Forced-deviation responses of the learned PPO policy with a deviation at $t=1$. Left: fixed $\lambda$. Right: exponentially increasing $\lambda$.}
  \label{fig:ppo_forced_deviation_compare}
\end{figure}

\subsection{Rate of Change in Demand Scale and Converged Collusion}
...
By contrast, DQN relies on off-policy one-step TD learning with a replay buffer, which mixes transitions from high-demand and low-demand periods and therefore makes it more difficult to capture the time-dependent value of inventory allocation.
In scenarios where demand is concentrated in the later periods, the gradients from the later steps dominate the trajectory-based updates, which hinders the learning of early-stage coordination and thereby lower the collusion index.
...
Under DQN, by contrast, the response to the same environmental variation is comparatively small, suggesting that the learning style of the algorithm, in particular, whether it is on-policy or off-policy, fundamentally affects how agents adapt to demand variation.
%
These findings suggest that temporal demand patterns in reservation markets are not merely a matter of allocating sales opportunities over time, but also serve as environmental factors that affect the sustainability of algorithmic collusion.
Real airline-ticket and hotel-booking markets inevitably involve seasonality and temporal demand variation, and such temporal structure, therefore, affects competition under algorithmic pricing.
...
One possible future work is to construct more realistic market environments by combining realistic assumptions on quality, horizontal differentiation, and inventory constraints, and examine the implications of algorithmic collusion for real-world markets.
